...
import com.bresee.breseefaceapitest.FaceSDKManagerSingleton;
import com.bresee.breseefaceapitest.constant.BaseConstant;
...
import com.bresee.breseefaceapitest.utils.SerialPortUtils;
    ...
    private final Runnable connectRunnable = new Runnable() {
        ...
        public void run() {
            try {
                ...
                if (mSocket == null) {
                    mSocket = new Socket();
                    mSocket.connect(new InetSocketAddress("192.168.0.151", 8099), 10);
...
//            int count = mBis.available();
                ...
                while ((size = mBis.read(buffers)) != -1) {
                    ...
                    if (size == 14) { // size == 14说明收到心跳包
                        ...
                        if (!Utils.byteToHex(checkByte).equals("0000")) {
                            ...
                            if ((size - 14) % 16398 == 0) { //没有余数说明是完整的包
                                registerStartAddress(buffer, size);
                            } else {
                                //合并两个byte数组
    ...
    private void registerStartAddress(byte[] buffer, int size) {
        ...
        //获取特征库 寄存器起始地址
            ...
            if (inTotalInfoBool) {
                //总人员信息
                inTotalInfo = faceNum + (size - 14) / 16398;
    ...
    private void onRegister(final byte[] data, final int size) {
...
//        Log.e(TAG, "校验码: " + checkCodeNum);
        ...
        singleThreadExecutor.execute(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 1; i <= personnelInfoSize / 16398; i++) {

                    byte[] personnelInfoByte = new byte[personnelInfoSize];
                    System.arraycopy(data, 12, personnelInfoByte, 0, personnelInfoSize);

                    int j = personnelInfoSize / (personnelInfoSize / 16398);

                    byte[] featureByte = new byte[16384];
                    ...
                    if (i == 1) {
                        //人脸信息
                        System.arraycopy(personnelInfoByte, 0, featureByte, 0, 16384);
                        //卡号
                        ...
                        if (j * (i - 1) <= personnelInfoSize) {
                            System.arraycopy(personnelInfoByte, j * (i - 1), featureByte, 0, 16384);
                        }
                        //卡号
                        System.arraycopy(personnelInfoByte, j * (i - 1) + 16384, cardByte, 0, 4);
                        //人名
                        System.arraycopy(personnelInfoByte, j * (i - 1) + 16388, nameByte, 0, 10);
                    }
                    ...
                    String userInfo = username + card;

                    //加载特征到缓存接口
                    int mFeatureFileState = faceSDKManager.isfCacheAddFeature(ConfigParam.faceLibName, userInfo, featureByte);
                    if (mFeatureFileState == 0) {
                        success++;
                        Log.e(TAG, "加载特征到缓存接口: " + success);
                    }
                }
    ...
    private void newStaff(byte[] data) {
        ...
        System.arraycopy(data, 12, featureByte, 0, 16384);
...
//                }
    ...
    private void sdkDeleteFace(String faceInfo) {
        Log.e(TAG, "removeStaff: 卡号：" + faceInfo);
        //从缓存删除人脸特征接口
        int deleteFaceDbState = faceSDKManager.isfCacheDeleteFeature(ConfigParam.faceLibName, faceInfo);
        if (deleteFaceDbState == 0) {
            Log.e(TAG, "sdkDeleteFace: 删除成功");
        } else {
            Log.e(TAG, "sdkDeleteFace: 删除失败");
        }
    }
    ...
    private void sdkClearLibray() {
        ...
        if (clearFaceDbState == 0) {
            Log.e(TAG, "sdkClearLibray: 清库成功");
        } else {
            Log.e(TAG, "sdkClearLibray: 清库失败");
        }
    }
